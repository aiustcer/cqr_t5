...
from t5_response.model import cqr_t5
...
def get_command():
    ...
    parser.add_argument("--output_dir", default='models/query-rewriter-self-learn-bs2-e1-0', type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument("--model_name_or_path", default="t5-base", type=str,
                        help="The model checkpoint for weights initialization.")
    parser.add_argument("--block_size", default=500, type=int,
                        help="Optional input sequence length after tokenization."
                             "The training dataset will be truncated in block of this size for training."
                             "Default to the model max input length for single sentence inputs (take into account special tokens).")
    parser.add_argument("--train_file", default='data/weak_data_div/self-learn.jsonl.0', type=str,
...
def collate_fn(batch_dataset: list):
    ...
    for example in batch_dataset:
        ...
        return_tuple[2].append(example.input)
        return_tuple[3].append(example.label)
...
def train(args, train_dataset, model, tokenizer, logger, cross_validate_id=-1):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            input_ecodings = tokenizer.batch_encode_plus(inputs, return_tensors='pt', padding=True)
            label_ecodings = tokenizer.batch_encode_plus(labels, return_tensors='pt', padding=True)

            input_ecodings = input_ecodings.to(args.device)
            label_ecodings = label_ecodings.to(args.device)

            input_ids = input_ecodings['input_ids']
            attention_mask = input_ecodings['attention_mask']

            lable_ids = label_ecodings['input_ids']
            ...
            data_writer.add_scalar('loss', loss.item(), step, display_name='loss')
...
def main():
    ...
    config_class, model_class, tokenizer_class = T5Config, cqr_t5, T5Tokenizer

    if not args.cross_validate:
        config = config_class.from_pretrained(args.model_name_or_path)
        tokenizer = tokenizer_class.from_pretrained(args.model_name_or_path)
        tokenizer.add_special_tokens(special_tokens_dict)
        model = model_class.load(args.model_name_or_path)
        model.t5.resize_token_embeddings(len(tokenizer))  # resize
        ...
        for i in range(NUM_FOLD):
            ...
            tokenizer.add_special_tokens(special_tokens_dict)
            model = model_class.load(args.model_name_or_path + suffix)
            model.t5.resize_token_embeddings(len(tokenizer))  # resize
            model.to(args.device)
